...
function Logo() { return <Link to="/" className="brand-mark font-bold tracking-[-.05em] text-xl">EXCLUSIVE FASHION WEAR</Link> }
...
function Shop(){
  ...
  const fallbackCategories = [...categories, ...womensCategories];
  useEffect(() => {
    if (!supabase) return;
    supabase.from('categories').select('slug,name,image_url').order('name')
      .then(({ data }) => setDbCategories(data || []));
  }, []);
  const liveCategories = dbCategories.map(({ slug, name, image_url }) => [slug, name, image_url]);
  const categoryList = liveCategories.length ? liveCategories : fallbackCategories;
  return <section className="bg-paper px-[5.5vw] py-20"><p className="font-mono text-[10px] uppercase tracking-[.13em]">All categories</p><h1 className="mt-4 font-bold leading-none tracking-[-.08em] text-[clamp(56px,9vw,128px)]">SHOP <em className="font-serif font-medium">THE DROP.</em></h1><div className="mt-16 grid gap-px bg-tan sm:grid-cols-2 lg:grid-cols-3">{categoryList.map(([slug,name,img])=><Link to={`/shop/${slug}`} key={slug} className="group bg-paper p-5">{img ? <img className="h-96 w-full object-cover transition group-hover:opacity-80" src={img} alt={name}/> : <div className="grid h-96 place-items-center bg-tan font-mono text-xs tracking-[.12em] text-ink/55">{name}</div>}<div className="flex items-center justify-between pt-5"><h2 className="text-xl font-bold tracking-[-.05em]">{name}</h2><span>→</span></div></Link>)}</div></section>
}
function ProductCategory(){
  ...
  useEffect(() => {
    ...
    async function loadCategory() {
      if (!supabase) { if (active) setStatus('error'); return; }
      setStatus('loading');
      const { data: categoryData, error: categoryError } = await supabase.from('categories').select('id,name').eq('slug', slug).maybeSingle();
      if (categoryError || !categoryData) { if (active) setStatus('missing'); return; }
      const { data: productData, error: productError } = await supabase.from('products').select('id,name,slug,price,image_url').eq('category_id', categoryData.id).eq('is_active', true).order('created_at', { ascending: false });
      if (!active) return;
      setCategory(categoryData);
      setProducts(productData || []);
      setStatus(productError ? 'error' : 'ready');
    }
    loadCategory();
    return () => { active = false; };
  }, [slug]);
  const title = category?.name || slug.replace(/-/g, ' ');
  return <section className="min-h-[570px] bg-paper px-[5.5vw] py-20"><Link className="font-mono text-[10px] tracking-[.1em]" to="/shop">← ALL COLLECTIONS</Link><p className="mt-10 font-mono text-[10px] uppercase tracking-[.13em] text-rust">Collection</p><h1 className="mt-4 font-bold leading-none tracking-[-.08em] text-[clamp(52px,8vw,110px)]">{title}</h1>{status === 'loading' && <p className="mt-12 font-mono text-xs text-ink/60">LOADING PRODUCTS...</p>}{status === 'missing' && <p className="mt-12 text-sm">This category could not be found.</p>}{status === 'error' && <p className="mt-12 text-sm text-rust">Products could not be loaded. Please refresh and try again.</p>}{status === 'ready' && (products.length ? <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{products.map(product => <article key={product.id} className="group"><img className="h-[430px] w-full bg-tan object-cover transition group-hover:opacity-85" src={product.image_url} alt={product.name}/><div className="flex items-start justify-between gap-4 pt-4"><div><h2 className="text-xl font-bold tracking-[-.05em]">{product.name}</h2><p className="mt-1 font-mono text-[11px] text-ink/60">£{Number(product.price).toFixed(2)}</p></div><span className="font-mono text-xs text-rust">VIEW →</span></div></article>)}</div> : <p className="mt-12 text-sm text-ink/65">No products have been added to this category yet.</p>)}</section>
}
